...
import { upateImgJsonById, updateImgJsonAndAudit, getAuditById, updateAuditById, getHwStuHomeworkPicList, deleteImgJsonAndAudit, isReceive, updateRepeatImgById, deleteRepeatImgById, getAuditByImgId } from '@/api/homework/hwHomework'
...
export default {
  ...
  methods: {
        ...
        image.addEventListener('click', (event) => {
          if (that.updateImgJsonContro) {
            ...
            if (that.auditControl) {
              ...
              upateImgJsonById({ id: that.imgId, imgJson: jsonStr }).then(res => {
      ...
      if (this.imgs.length > 0) {
        ...
        if (this.updateImgJsonContro) {
          ...
          if (this.auditControl) {
            ...
            upateImgJsonById({ id: this.imgId, imgJson: im.imgJson }).then(res => {
              if (res.success && res.data) {
                this.imgSWitch(this.imgs[idx - 1].uid, idx)
    ...
    updateComments({ data }) {
      // 此处需要判断是新建还是编辑
      updateAuditById({ id: data.id, tagComment: data.tagComment }).then(res => {
        if (res.success && res.data) {
    ...
    updateTagging(tagComment) {
      updateAuditById({ id: this.tagParagraph, tagComment: tagComment }).then(res => {
        if (res.success && res.data) {
          res.data.tagComment = this.unescape(res.data.tagComment)
          res.data.repeatComment = this.unescape(res.data.repeatComment)
          res.data.head = true
    ...
    updateJson() {
      ...
      upateImgJsonById({ id: that.imgId, imgJson: jsons }).then(ress => {
    ...
    addTagging(json, tagComment) {
      ...
      updateImgJsonAndAudit({ hwStuHomeworkPicImgJsonQueryParam: { id: this.imgId, imgJson: '' }, hwHomeworkAuditPic: { stuHomeworkId: this.hwStuHomeworkId, tagParagraph: this.tagParagraph, tagComment: tagComment }}).then(res => {
        if (res.success && res.data) {
          ...
          json = that.getNewJson(json)
          const jsonStr = JSON.stringify(json)
          upateImgJsonById({ id: that.imgId, imgJson: jsonStr }).then(ress => {
            if (ress.success && ress.data) {
    ...
    deleteComment({ data, success, error }) {
      ...
      deleteImgJsonAndAudit({ hwHomeworkAuditPic: { id: data.id }, hwStuHomeworkPicImgJsonQueryParam: { id: this.imgId, imgJson: jsonStr }}).then(res => {
        if (res.success && res.data) {
