...
import { getHwStuHomeworkPicListShare, getAuditByImgIdShare, getAuditByIdShare } from '@/api/homework/hwHomework'
...
export default {
  ...
  created() {
    ...
    this.imgId = imgId
    getHwStuHomeworkPicListShare(this.hwStuHomeworkId).then(res => {
      if (res.success) {
  ...
  methods: {
    ...
    upClick() {
      ...
      if (this.imgs.length > 0) {
        ...
        if (this.updateImgJsonContro) {
          ...
          this.imgSWitch(this.imgs[idx - 1].uid)
          that.currentNumber = idx
          /*            const elImage = document.querySelectorAll('.el-image')
                elImage.forEach(function(img, b) {
                  img.classList.remove('imgclass')
                })
                elImage[idx - 1].classList.add('imgclass')*/
          this.imgId = this.imgs[idx - 1].uid
          this.updateImgJsonContro = true
        } else {
      ...
      if (this.imgs && this.imgs.length > 0) {
        ...
        if (img.imgJson) {
          ...
          this.$refs.canvas.load(jsonStr, function() {
            getAuditByImgIdShare(id).then(res => {
    ...
    mousemove(e) {
      ...
      if (e.e.offsetX === undefined) {
        ...
        wz = { x: e.e.changedTouches[0].clientX, y: e.e.changedTouches[0].clientY }
        xyor.x = e.pointer.x
        xyor.y = e.pointer.y
      } else {
    ...
    selected(e) {
      ...
      if (this.type === 'remove') {
        ...
        cc.style.right = window.innerWidth - (e.e.pageX ? e.e.pageX : e.e.touches[0].clientX) + 'px'
        cc.style.top = (e.e.pageY ? e.e.pageY : e.e.touches[0].clientY) + 10 + 'px'
        this.removeObject = e
        return false
      }
      // 如果点击了标记点
