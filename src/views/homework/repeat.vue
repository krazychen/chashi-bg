  ...
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="tableData" style="width: 100%" :default-sort="{prop: 'date', order: 'descending'}" :row-class-name="tableRowClassName" @filter-change="fnFilterChangeInit">
      ...
      <el-table-column label="操作" width="245" fixed="right" align="center">
        ...
        <template slot-scope="scope">
          <!--批改：首先判断是否批改，未批改分为一类-->
          <el-button v-if="scope.row.auditState === '待批改' || scope.row.auditState === '批改超期'" type="primary" size="small" @click="seeShare('/system/'+scope.row.stuHomeworkId+'/showRepeatAudit/0',scope.row.stuHomeworkId+'reapg')">去查看</el-button>
          <!--复评：排除以上两个状态，判断复评状态，为0的情况下可以进行复评-->
          <el-button v-else-if="scope.row.repeatState === '0'" type="primary" size="small" @click="seeShare('/system/'+scope.row.stuHomeworkId+'/showRepeatExamine/0',scope.row.stuHomeworkId+'reafp')">去查看</el-button>
          <!--查看：否则就是查看-->
          <el-button v-else type="primary" size="small" @click="seeShare('/system/'+scope.row.stuHomeworkId+'/repeatDetails/0',scope.row.stuHomeworkId+'reack')">去查看</el-button>
        </template>
      </el-table-column>
    </el-table>
